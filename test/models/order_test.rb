...
class OrderTest < Minitest::Test
        ...
        def test_get_customer_order(customer_id)
                customer_order = @customer.get_customer_order(1)
                # test returns an array
                assert_instance_of(Array, customer_order)
        end

        # test add payment to order by customer and payment id
        def test_add_payment_to_order(customer_id, payment_id)
                order_payment = @payment_type.add_payment_type_to_order(1, 1)
        ...
        def add_product_to_order(customer_id)
                new_order_product_id = @product.add_product_to_order
